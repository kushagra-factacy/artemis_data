...
nlp_ic = spacy.load(r"ic_model1")
sector = pd.read_excel("subsec-to-use.xlsx")
import os
import numpy as np
...
query = f"SELECT c.id,c.title,c.published_date, c.published_date_time,c.author,c.url,c.content_cl FROM c WHERE c.published_date= '{today}'"
items = container.query_items(query, enable_cross_partition_query=True)
item_list = list(items)
input_df = pd.DataFrame(item_list)
...
client_upsert = CosmosClient(COSMOS_DB_URI, credential=COSMOS_DB_KEY)
container_upsert = client_upsert.get_database_client(DATABASE_NAME).get_container_client(CONTAINER_NAME)
query_last_inserted = "SELECT TOP 1 c.Art_Id FROM c ORDER BY c._ts DESC"
result_last_inserted = list(container_upsert.query_items(query_last_inserted, enable_cross_partition_query=True))
if result_last_inserted:
    last_inserted_art_id = result_last_inserted[0]['Art_Id']
    df = pd.DataFrame(columns=["Art_Id", "content", "published_date"])
    for idx, row in input_df.iterrows():
        art_id, content, date = row["id"], row["content_cl"], row["published_date"]
        if art_id > last_inserted_art_id:
            row_df = pd.DataFrame({"Art_Id": [art_id], "content": [content], "published_date": [date]})
            df = pd.concat([df, row_df], ignore_index=True)
else:
    print("No records found in the container.")
...
for idx, row in df.iterrows():
    ID, content = row["Art_Id"], row["content"]
    date = row["published_date"]
...
finale = trail_df[~((trail_df['org'].str.len() < 2) | (trail_df['ic_got'].str.len() < 2))]
...
#vguj# except exceptions.CosmosHttpResponseError as e:
#     print(f"Failed to upsert. Error: {e}")
