...
cf = new_df.explode('Art_Id')
data = cf["Art_Id"].to_list()
...
tf.rename(columns = {'id':"Art_Id"},inplace = True)
df = pd.merge(tf, cf[['Art_Id', 'Corrected_Investee']], on='Art_Id', how='left')
...
def ic_picked(article):
    ...
    pattern = [{"POS": "ADJ", "OP": "?"}, {"POS": "NOUN"}, {"LOWER": {"IN": ["chain","giant","providers","provider","firm","start-up","company", "startup", "platform","app","services","start-up"]}}]
...
def find_matching_word(ic_picked_word):
    words_to_check = ["providers","provider","firm","start-up","company", "startup", "platform","app","services","start-up","giant"]
...
def investee_sentences(paragraph, target_string):
    sentences = paragraph.split('.')
    result_sentences = [sentence.strip() + ' ' for sentence in sentences if target_string.lower() in sentence.lower()]
    return ''.join(result_sentences).strip() if result_sentences else None


def reconfirm_InvesteeIC_got(content, investee):
    if not content or not investee:
        return "", ""
    context = investee_sentences(content, investee)
    if context is None:
        return "", ""
    SCORE_THRESHOLD = 0.1
    def get_answer(question):
        ...
        res = nlp_qa(QA_input)
        return res['answer'] if res.get('score', 0) >= SCORE_THRESHOLD else ''
    investee_ic = get_answer(f"{investee} is a")
    picked_investee = get_answer(f"{investee_ic} is which company")
    picked_investee = picked_investee.replace("\n", "").replace("\n\n", "")
    investee_ic = investee_ic.replace("\n", "").replace("\n\n", "")
    if picked_investee == investee:
        return picked_investee, investee_ic
    else:
        return "", ""
...
def investeeIC_process(content,org):
    result_org = org.split('.')[0]
    picked_investee, investee_ic = reconfirm_InvesteeIC_got(content,result_org)
    predict = factacy_prediction(investee_ic)
    ic_got_list = []
    investee_got = {}
    investee_got["org"] = picked_investee
    investee_got["investee_ic"] = investee_ic
    investee_got["factacy_classification"] = predict
    if investee_ic and not predict:
      investee_got["Manual Check"] = "Imporve bag of words"
    ic_got_list.append(investee_got)
    return ic_got_list
...
for idx, row in df.iterrows():
    ID, content = row["Art_Id"], row["content_cl"]
    date = row["published_date"]
...
final_df = final_df.rename(columns = {"org" :"Corrected_Investee"})
final_df.columns
merged_df = pd.merge(cf, final_df, on='Corrected_Investee', how='left', indicator=True)
result_df = merged_df[merged_df['_merge'] == 'left_only']
result_df = result_df.drop(columns=['_merge']).reset_index(drop=True)
# semi_df = result_df.drop(columns = ['Art_Id_y', 'ic_got', 'Factacy_BOW_Classification', 'published_date_y'])
semi_df = result_df.dropna(axis = 1, how = "any")
print("semi_df columns",semi_df.columns)

        
another_lf = pd.DataFrame(columns = ["Art_Id","Ic_result","published_date"])
for idx, row in semi_df.iterrows():
    ID, content = row["Art_Id_x"], row["content_cl"]
    date, investee = row["published_date_x"] , row["Corrected_Investee"]
    ic_result = investeeIC_process(content, investee)
    entry = pd.DataFrame({"Art_Id":[ID],"Ic_result":[ic_result],"published_date":[date]})
    another_lf = pd.concat([another_lf, entry], ignore_index = True)

almost_df = pd.concat([lf, another_lf],ignore_index = True)
trail_df = data_formating(almost_df)
...
classifier.fit(X, y)
ic_got = trail_df["ic_got"]
published_date = trail_df["published_date"]
...
done_df = pd.DataFrame({
    'Art_Id': trail_df['Art_Id'],
    'org': trail_df["org"],
    'ic_got': ic_got,
    "Factacy_BOW_Classification": trail_df["Factacy_BOW_Classification"],
    'Published_Date': published_date,
    'Predicted': classifier.predict(X_new),
    'Probability': predicted_probabilities
})
done_df[['factacy_main_sector', "factacy_subsector1", "factacy_subsector2", "factacy_subsector3"]] = done_df['Predicted'].str.split('_', expand=True)
done_df = done_df.drop(columns = ["Predicted"])

finale = done_df[~((done_df['org'].str.len() < 2) | (done_df['ic_got'].str.len() < 2))]
...
try:
    ...
    json_data = finale.to_json(orient='records')
